...
def select_chart(choose):
    if choose == 'All Sites':
        ...
        df = spacex_df[spacex_df["Launch Site"] == choose]["class"].value_counts().to_frame()
        df["name"] = ["Failure", "Success"]
        fig = px.pie(df, values='class', names='name', title='Total Success Launches for ' + choose)
    return fig
...
def scatter_chart(sdd, pls):
    print(sdd)
    print(pls)
    if sdd == 'All Sites':
        df_1 = spacex_df
        df_2 = df_1[df_1["Payload Mass (kg)"] >= pls[0]]
        df_3 = df_2[df_1["Payload Mass (kg)"] <= pls[1]]
        fig2 = px.scatter(df_3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    else:
        df_1 = spacex_df[spacex_df["Launch Site"] == sdd]
        df_2 = df_1[df_1["Payload Mass (kg)"] >= pls[0]]
        df_3 = df_2[df_1["Payload Mass (kg)"] <= pls[1]]
        fig2 = px.scatter(df_3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    return fig2
